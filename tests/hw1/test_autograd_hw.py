...
def gradient_check(f, *args, tol=1e-6, backward=False, **kwargs):
    ...
    for i in range(len(args)):
        assert (numerical_grads[i].shape == computed_grads[i].shape)
    error = sum(
...
def test_matmul_batched_backward():
    ...
    gradient_check(
        ndl.matmul,
        ndl.Tensor(np.random.randn(2, 2, 5, 4)),
        ndl.Tensor(np.random.randn(1, 4, 3)),
    )
    gradient_check(
        ndl.matmul,
        ndl.Tensor(np.random.randn(2, 2, 1, 2)),
        ndl.Tensor(np.random.randn(2, 1, 2, 1)),
    )
...
def test_compute_gradient():
    ...
    # my test
    gradient_check(
        lambda A, B: ndl.summation(A + B, axes=None),
        ndl.Tensor(np.random.randn(10, 9)),
        ndl.Tensor(np.random.randn(10, 9)),
        backward=True,
    )
